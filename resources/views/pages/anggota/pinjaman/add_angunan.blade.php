        ...
        <form action="{{ route('pinjaman.store') }}" method="POST" enctype="multipart/form-data">
            ...
            <div class="form-group">
                <input type="hidden" id="jenis_pinjaman" name="jenis_pinjaman" class="form-control" required
                    value="{{ old('jenis_pinjaman', 'Pinjaman Dengan Angunan') }}" readonly>
            </div>
            <div class="form-group">
            ...
            <div class="form-group">
                <label for="jenis_angungan">Jenis Angunan</label>
                <select id="jenis_angungan" name="jenis_angungan" class="form-control" required>
                    <option value="">Jenis Angunan</option>
                    @foreach ($angunan as $item)
                        <option value="{{ $item }}" {{ old('jenis_angungan') == $item ? 'selected' : '' }}>
                            {{ ucfirst($item) }}</option>
                    @endforeach
                </select>
            </div>
    ...
    <script>
        ...
        function previewImage() {
            ...
            if (imageInput.files && imageInput.files[0]) {
                ...
                // Fungsi untuk mengatur src gambar ke preview
